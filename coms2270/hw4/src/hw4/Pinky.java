...
public class Pinky extends Ghost {
	...
	protected Location getTarget(Descriptor d) {
		...
		if (mode == Mode.CHASE) {
			Direction pacDir = d.getPlayerDirection();
			int pacRow = d.getPlayerLocation().row();
			int pacCol = d.getPlayerLocation().col();
			
			if (pacDir.equals(UP)) {
				pacRow -= 4;
			}
			else if (pacDir.equals(DOWN)) {
				pacRow += 4;
			}
			else if (pacDir.equals(LEFT)) {
				pacCol -= 4;
			}
			else {
				pacCol += 4;
			...
			return new Location (pacRow, pacCol);
		}
